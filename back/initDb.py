...
user_id = 1
for user in users:
    cur.execute("INSERT INTO user (username, pass, email, first_name, last_name,gender, sexual_preference, birthdate, latitude, longitude) VALUES (?, ?, ?, ?, ?, ?, ?)",
            (user['name'], user['pass'],user['email'], user['first_name'], user['last_name'], user['gender'], user['sexual_preference'], user['birthdate'], user['latitude'], user['longitude'], ))
    image_name = '/pictures/' + user['image_name']
    with open(image_name, "rb") as file:
        file_content = file.read()
        encoded_content = base64.b64encode(file_content)
    cur.execute("INSERT INTO image (image_file, user_id, is_profile_picture) VALUES (?, ?, ?)",
                ((encoded_content), str(user_id), str(True),))
    user_id += 1
