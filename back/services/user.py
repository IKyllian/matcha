...
def getUserWithImagesById(user_id):
    response = makeRequest("SELECT id, username, first_name, last_name, email, gender, sexual_preference, bio, fame_rating, birth_date, latitude, longitude FROM user WHERE id = ?", (str(user_id),))
    user = response[0]
