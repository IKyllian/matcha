...
def getUserWithProfilePictureById(user_id):
    result = makeRequest("SELECT id, username, first_name, last_name, email, gender, sexual_preference, bio, fame_rating, birth_date, latitude, longitude, position_name, is_connected, last_connection, is_activated, is_valid FROM user WHERE user.id = ?", (str(user_id),))
    user = result[0]
