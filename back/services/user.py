...
def getUserWithProfilePictureById(user_id):
    result = makeRequest("SELECT user.id, username, first_name, last_name, gender, sexual_preference, birth_date, is_connected, last_connection, is_activated FROM user WHERE user.id = ?", (str(user_id),))
    user = result[0]
